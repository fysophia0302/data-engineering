# Retail ETL Pipeline — Teradata to BigQuery
...
A multi-stage ETL pipeline that extracts retail sales and product data from Teradata,
...
```
retail-etl-teradata-to-bigquery/
...
└── stage2_product_flag/
    └── product_flag_etl.py              # product classification flag extraction
```

## Pipeline Stages

### Stage 1 — Regional Sales Extraction
- Spawns one worker process per region (up to 4 in parallel)
...
### Stage 2 — Product Flag Extraction
- Extracts product classification and regulatory flag data from Teradata
...
This pipeline is designed for a proprietary retail data environment.
